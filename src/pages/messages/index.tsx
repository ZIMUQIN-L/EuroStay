...
const Index = () => {
  ...
  const fetchMessages = async () => {
    ...
    setLoading(true);
    try {
      const response = await API.messages.getSessionList(page);
      const result = response || { data: [], total: 0, per_page: 0, current_page: 0, last_page: 0 };
      
      const sessionItems = result.data || [];
      
      const normalMsgs: MessageItem[] = [];
      const systemMsgs: MessageItem[] = [];
      const strangerMsgs: MessageItem[] = [];
  
      sessionItems.forEach((record) => {
        ...
        const messageObj: MessageItem = {
          ...
          avatar: otherAvatar || 'https://eurostay-1330475057.cos.eu-frankfurt.myqcloud.com/sys/loading.png',
      ...
      // 对消息按时间排序，越新的越靠前
      const sortedMessages = [...messages, ...normalMsgs].sort((a, b) => {
        const timeA = a.rawData?.createTime ? new Date(a.rawData.createTime).getTime() : 0;
        const timeB = b.rawData?.createTime ? new Date(b.rawData.createTime).getTime() : 0;
        return timeB - timeA; // 降序排序，新的在前
      });
      setMessages(sortedMessages);

      const sessionDict: Record<number, MessageItem> = {};
